...
import code.PseudoLabeler as pl
...
class SemiSupervisedLearner:
    ...
    def bayes_classifier(self, distributions=pm.NormalDistribution):
        model = BayesClassifier.from_samples(distributions, X=self.X, y=self.y)
        return model.score(self.X_t, self.y_t)
    ...
    def pseudo_labeling(self, alg, sample_rate=0.2):
        ...
        X_u = self.X[idx]
        model = pl.PseudoLabeler(model=alg, X_t=X_u, sample_rate=sample_rate)
        model.fit(self.X, self.y)
        return model.score(self.X_t, self.y_t)
